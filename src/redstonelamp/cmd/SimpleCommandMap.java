...
public class SimpleCommandMap<T> implements CommandMap {
	 ...
	 * @return boolean
	 */
	public boolean register(String label, Command command, boolean isAlias,
			String prefix) {
	...
	public boolean dispatch(CommandSender sender, String commandLine) {
		...
		try {
			target.execute(sender, sentCommandLabel,
					(String[]) Arrays_copyOfRange(args, 1, args.length));
		} catch (Exception ex) {
			ex.printStackTrace();
		} catch (Throwable ex) {
			ex.printStackTrace();
			throw new IllegalArgumentException(
					"Unhandled exception executing '" + commandLine + "' in "
							+ target, ex);
		}
		return true;
	}
	...
	private T[] Arrays_copyOfRange(String[] args, int start, int end) {
		if (args.length >= start && 0 <= start) {
			if (start <= end) {
				...
				T[] copy = (T[]) Array.newInstance(args.getClass()
