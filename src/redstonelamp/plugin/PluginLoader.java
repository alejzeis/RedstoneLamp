...
public class PluginLoader implements Listener {
		...
		for (final Method method : methods) {
			...
			if (method.getParameterTypes().length != 1
					|| !Event.class.isAssignableFrom(checkClass = method
							.getParameterTypes()[0])) {
				// RedstoneLamp.server.getLogger().error(plugin.getName() +
			...
			eventSet.add(new RegisteredListener(listener, executor, ep, plugin,
					false));
	...
	private void generatePluginJavaClassFile(final File file) {
		...
		JavaCompiler.CompilationTask task = compiler.getTask(null, fileManager,
				plistener, options, null, compilationUnits);
		...
		if (success) {
			...
			RedstoneLamp.server
					.getLogger()
					.info(
							" :Compilation Success! Class files are generated in in-use folder");
		} else {
